...
import com.ming.stock.mapper.SysPermissionMapper;
import com.ming.stock.mapper.SysRolePermissionMapper;
...
public class PermissionServiceImpl implements PermissionService {
    ...
    public R<List<PermissionRespNodeVo>> selectAllTree() {
        ...
        //递归组装权限树状结构 不包含按钮
        List<PermissionRespNodeVo>  tree= getTree(permissions,"0",false);
        return R.ok(tree);
    }
    ...
    public R<String> removePermission(String permissionId) {
        ...
        int updateCount = sysPermissionMapper.updateByPrimaryKeySelective(permission);
        if (updateCount!=1) {
            throw new BusinessException(ResponseCode.ERROR.getMessage());
        }
        return R.ok(ResponseCode.SUCCESS.getMessage());
